...
class BBTopo(Topo):
    ...
    def build(self, n=2):
        ...
        # Define required params for generating our links
        bwHost = args.bw_host
        bwNet = args.bw_net
        delay = args.delay
        maxq = args.maxq
        ...
        self.addLink(h1, switch, bw=bwHost, delay='%sms'%delay, max_queue_size=maxq)

        # Establish link between router and h2 (bottleneck)
        self.addLink(switch, h2, bw=bwNet, delay='%sms'%delay, max_queue_size=maxq)
...
def start_ping(net):
    ...
    # until stdout is read. You can avoid this by running popen.communicate() or
    # redirecting stdout
